...
exports.setupTeamHandlers = void 0;
const User_1 = __importDefault(require("../../../models/User"));
const userStore_1 = require("../../store/userStore");
const setupTeamHandlers = (socket, io, activeUsers) => {
    socket.on('create_team', async (data) => {
        try {
            ...
            if (userStore_1.userTeams.has(userId)) {
            ...
            userStore_1.activeTeams.set(teamId, team);
            userStore_1.userTeams.set(userId, teamId);
            socket.join(teamId);
    ...
    socket.on('send_team_invite', async (data) => {
        try {
            ...
            if (userStore_1.userTeams.has(receiverId)) {
    ...
    socket.on('leave_team', async (data) => {
        try {
            ...
            const teamId = userStore_1.userTeams.get(userId);
            if (!teamId) {
            ...
            userStore_1.userTeams.delete(userId);
            socket.leave(teamId);
    ...
    socket.on('send_team_message', async (data) => {
        try {
            ...
            const teamId = userStore_1.userTeams.get(senderId);
            if (!teamId) {
                socket.emit('team_error', { message: 'Not in a team' });
                return;
            }
...
exports.setupTeamHandlers = setupTeamHandlers;
